...
public class XmlUtils<T> {
	 ...
	 * @param fields
	 *            字段集合一一对应节点集合
	 * @param elements    //d（这两行标红，是因为这两个是java中字段和xml文件中的字段对应）
	 *            节点集合一一对应字段集合
	 * @param itemElement
	 *            每一项的节点标签
	 * @return
	...
	public static void XmlSerializer(FileOutputStream fos, XmlElements data)
	{
		try {
			// 获取xml序列化器
	...
	private static void xmlPackt(XmlSerializer xs, XmlElements data)
	{
		if(xs == null||data == null)
			return;

		try {
			xs.startTag(data.tagNamespace, data.tagName);

			if(data.elements == null)
				return;

			for(Object object : data.elements)
			{
				if (object instanceof String)
				{
					xs.text((String)object);

				}else if(object instanceof XmlElements)
				{
					xmlPackt( xs, (XmlElements)object);
				}
			}
			xs.endTag(data.tagNamespace, data.tagName);
